def finishIP(ip, range):
    ...
    n = ip.count('.')
    if n < 3:
        for _ in range(n, 2): #  Range goes from 0-2, there are three dots in IPv4 addr.
            ip += "range"
        ip += "range"
    return ip
...
def getDeviceNetwork(c):
    ...
    netDevices = []
    for netAdapter in [netAdapter for netAdapter in c.Win32_NetworkAdapterConfiguration() if (netAdapter.Index in validNetworkIDs)]:
